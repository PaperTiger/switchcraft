...
class SwitcherifyTwigExtension extends \Twig_Extension
{
	...
	public function switcherifyFilter( $context, $currentIndex, $cases, $return = null ) {
		...
		if( is_array( $cases ) ) 
		{
			...
			{
				return $this->switcherifyCompiler( $context, $key, $case );
			}
		} else {
			return ( $return ) ? 
				$this->switcherifyCompiler( $context, $cases, $return ) : 
				$this->switcherifyCompiler( $context, $cases, $return, true ) ;
		}
	}
	 ...
	 * @return Mixed
	 */
	private function switcherifyCompiler( $context, $key, $case, $test = false )
	{

		$defaultParams = [ 'firstItem', 'lastItem', 'oddItem', 'evenItem' ];
		...
		if( in_array( $key, $defaultParams) ) 
		{
			switch ( $key ) 
			{
				...
				break;
				case 'lastItem':
					if( $context['loop']['index'] == $context['loop']['length'] ) return $this->switcherifyDecider( $case, $test );
				break;
				case 'oddItem':
					if( $context['loop']['index'] % 2 != 0 ) return $this->switcherifyDecider( $case, $test );
				break;
				case 'evenItem':
					if( $context['loop']['index'] % 2 == 0 ) return $this->switcherifyDecider( $case, $test );
				break;
			...
			{
				if( $context['loop']['index'] % $matches[1] == 0 ) return $this->switcherifyDecider( $case, $test );
			}
		}
	}
